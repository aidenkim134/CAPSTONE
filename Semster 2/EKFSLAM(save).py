...
class EKFSLAM:
    ...
    def __init__(self):
        ...
        def AssociateLandmark (self, Range, Bearing):
            ...
            Qt = np.array([[1, 0], [0, 1]])
            j = [np.where(self.u0 == 0)[0][0], np.where(self.u0 == 0)[0][0] + 1]
            Fxj = np.zeros([5, 3 + self.N_landmarks])
            Fxj[1,1] = 1;  Fxj[2,2] = 1; Fxj[0,0] = 1;
            Fxj[4, j[0]] = 1;  Fxj[5, j[1]] = 1
            ...
            S = np.array([Range * np.cos (Bearing + self.u0[2]), 
                                      Range * np.sin (Bearing + self.u0[2])])
            ...
            # self.u0[np.where(self.u0 == 0)[0][0]] = 
            # c[np.where(c == 0)[0][-1]] = 100
